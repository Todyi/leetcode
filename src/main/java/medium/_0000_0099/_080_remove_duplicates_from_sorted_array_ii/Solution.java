...
// with the first seven elements of nums being modified to0, 0, 1, 1, 2, 3 and3 respectively.
...
class Solution {
  ...
  //  内存消耗:39.1 MB,击败了51.13% 的Java用户
  public int removeDuplicates(int[] nums) {
    int idx = 0, len = nums.length, i = 0;
    boolean twice = false;
    while (true) {
      while (twice && i < len && nums[i] == nums[i - 1]) {
        i++;
      }
      if (len <= i) {
        break;
